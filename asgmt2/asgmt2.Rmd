...
```{r}
fit <- lm(milk~treatment + order, data = cow)
summary(fit)
```

As can be seen from the above figure, when treatment is B, it is inversely proportional to milk production.
...
```{r}
library(lme4)
fit <- lmer(milk~treatment + (treatment | order), data = cow)
summary(fit)
```

As can be seen from the above figure, when treatment is B, it is inversely proportional to milk production, but after adding a fixed effect, it can be found that their coefficient is -0.707.
...
```{r}
t.test(cow$milk[cow$treatment == "A"], cow$milk[cow$treatment == "B"], paire = TRUE)
```

As can be seen from the above figure, the p-value of the t-test is greater than 0.05, which is 0.8281, accepting the null hypothesis, so it is considered that there is no difference between them, only the impact of treatment on milk production is tested, and the impact of other factors on treatment is not considered.
