...
We plot the boxplot of time versus interface, and the interaction plot of interface.
...
with(data_search, interaction.plot(skill, interface, time))
with(data_search, interaction.plot(interface, skill, time))
```

Left plot: we can notice that different interfaces can lead to different time. 
Middle plot: lines are not parallel, which means interfaces have effect on time.
Right plot: lines are not parallel, which means treatment effects are different in each block.
...
Hence we reject $H_0$ and conclude that the search time is the same for all interfaces.
...
```{r}
with(data_search, 
     p_value <- friedman.test(time, interface, skill)$p.value)
```
...
Hence, we can't reject $H_0$, the factor interface is not significant. 
However, it's wrong to perform this test on this dataset, because these samples don't have equal variances. Moreover, there could exist interaction between two factors.
