...
3. $H_AB$ as $\gamma_{ij} = 0$ for all (i,j).
...
```{r}
summary(bread_lm)$coefficients
```

According to the analysis of variance and summary,
we can notice that $p$ of factor temperature is `r res$Pr[1]`, 
...
We also can see that the decay time decreases with the environment gets wetter and the temperature gets warmer. Moreover, the interaction of two factors stresses the explication.
...
As we see, the previous analysis says there are interaction, so we use addictive model to summary, which eliminates their interaction.
...
Nevertheless, we think this is not a good question due to the existence of interaction, since we hardly ignore the effect of interaction to the decay.
...
Left plot: normal is doubtful, because the line isn't straight enough. 
Right plot: the spread of residuals seems to be symmetric with fitted values, 
and some data points seem extreme, so there could exist outliers.
