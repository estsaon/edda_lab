...
par(mfrow=c(1, 2), mar = c(4, 4, 0, 2.5))
...
Right plot: lines are not parallel, which means interfaces have effect on time.
...
$\mu + \alpha_3 + \beta_3 =$  `r coef["(Intercept)"] + coef["interface3"] + coef["skill3"]`.
...
```{r}
p_value <- friedman.test(time, interface, skill, data=data_search)$p.value
```
