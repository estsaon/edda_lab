...
I<-3;J<-2;N<-3
random = rbind(rep(c("cold","intermedia","warm"), each=N*J),
      rep(c("dry","wet"), N*I), 
      sample(1:(N*I*J)))
```
...
par(mar = rep(2, 4));attach(data_bread)
...
For the third question, we set up $H_0$ as $\alpha_i = 0$ for all i; $H_0$ as $\beta_j = 0$ for all j; $H_0$ as $\gamma_{ij} = 0$ for all (i,j).


```{r}
factor_env = as.factor(environment); factor_hum = as.factor(humidity)
bread_lm = lm(hours~factor_env*factor_hum)
res = anova(bread_lm)
```

According to the analysis of variance, we can notice that $p$ of factor temperature is `r res$Pr[1]`, $p$ of factor humidity is `r res$Pr[2]`, and $p$ of two factors interaction is `r res$Pr[3]`. Since $p-values$ are less than $\alpha = 0.05$, so we reject $H_0$, that is to say both of factors and their interaction have great effect on the decay.
...
As we see, the previous analysis says there are interaction, so we use treatment model to summary.

```{r}
summary = summary(bread_lm)$coefficients
```

We find that the factors of wet humidity and warm temperature have the largest estimate values, so they have the greatest influence on the decay.
...
We check the normality and the assumption of equal variances. The left plot is the distribution of residuals, which should look normal. The right plot is the fitted plot versus residuals, there should not exist systematically pattern.

```{r, fig.height=3, fig.width=6, fig.align='center'}
par(mfrow=c(1,2))
qqnorm(residuals(bread_lm)); plot(fitted(bread_lm),residuals(bread_lm))
```

Left plot: normal is doubtful, because the line isn't straight enough. Right plot: the spread of residuals seems to be symmetric with fitted values, and some data points seem extreme, so there could exist outliers.
