...
#factor_feed = as.factor(treatment)
cow$treatment <- factor(cow$treatment); cow$per <- factor(cow$per);
milk_lm <- lm(milk~per+id+treatment, data=cow)
anova(milk_lm)
```
type of feedingstuffs have no influences on milk.

```{r}
summary(milk_lm)
```

the difference in milk production is -0.510.
...
milk_lm = lmer(milk~treatment+order+per+(1|id), REML = FALSE)
cow_summary = summary(milk_lm)

milk_lm1 = lmer(milk~order+per+(1|id), REML = FALSE)
anova(milk_lm1, milk_lm)
```

fixed effect has the same result with mixed effect. The different feeds have no influence on milk production.
...
the test is valid, it does have the same conclusion with a). since the test wash out cross-over effects, where there is no learning effect.
