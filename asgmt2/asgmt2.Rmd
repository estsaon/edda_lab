...
boxplot(time~factor_interface)
interaction.plot(factor_interface, factor_skill, time)
```

We notice that different interfaces can lead to different time.
...
So, interface 3 require the longest search time. The combination of skill level 2 and interface 2 requires the shortest search time.
