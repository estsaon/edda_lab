...
We let $\lambda$ to iterate from 0.01 to 0.1 by 0.001 to determine whether the data stem from the exponential distribution Exp($\lambda$) with some $\lambda$ from [0.01, 0.1]. Each of the simluations repeats 1000 times.
...
plot(lambdas, p_vals, type='l')
```

According to the plot, we can see that when $\lambda$ gets closer to `r lambdas[which.max(p_vals)]`, the $p$ can even reach more than 0.9. So the data must stem from the exponential distribution Exp($\lambda$) with some $\lambda$ from [0.01, 0.1].
