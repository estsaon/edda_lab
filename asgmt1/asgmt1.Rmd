...
R.Pan's solution
 ...
 We find an outlier in the sunflower group, since the 6th z-score in the group is lower than -2, the coincident weight is too small to count, thus we remove the outlier.

```{r}
    meatmeal <- subset(chickwts, feed == "meatmeal", select = c("weight"))
    sunflower <- subset(chickwts, feed == "sunflower", select = c("weight"))
    m_zscores <- scale(meatmeal$weight)
    s_zscores <- scale(sunflower$weight)
    sunflower <- sunflower[sunflower != 226]
...
Paired T-test:
```{r}
    p_value <- t.test(meatmeal, sunflower, paired = TRUE)$p.value
```

For $\alpha = 0.05$, $p$ = `r p_value`. 
Since $p$ < $\alpha$, we reject $H_0$ and support the claim. 2 samples are not paired.

<!--
Yuxiang Wang's solution:
...
Mann-Whitney test:
```{r}
    p_value <- wilcox.test(meatmeal, sunflower)$p.value
```

For $\alpha = 0.05$, $p$ = `r p_value`. 
Since $p$ < $\alpha$, we reject $H_0$ and support the claim. The distribution of meatmeal is shifted right from the sunflower.

Kolmogorov-Smirnov test:
```{r}
    p_value <- ks.test(meatmeal, sunflower)$p.value
```

For $\alpha = 0.05$, $p$ = `r p_value`. 
Since $p$ > $\alpha$, we failed to reject $H_0$.
...
one-way ANOVA:
```{r}
    linear_mode <- lm(weight~feed, data <- chickwts)
    p_value <- anova(linear_mode)$p.value
```

For $\alpha = 0.05$, $p$ = `r p_value`. 
Since $p$ < $\alpha$, we reject $H_0$ and support the claim. Factor feed is significant.

```{r}
    summary(linear_mode)$coefficients
```

Since feedsunflower has the highest t-value and p-value, we can conlude that sunflower feed is the best feed supplement.
<!--
Yuxiang Wang's solution:
...
-->
```{r}
    chickwts_data <- chickwts
    attach(chickwts_data)
    p_value <- kruskal.test(weight, feed)$p.value
```

For $\alpha = 0.05$, $p$ = `r p_value`. 
Since $p$ < $\alpha$, we reject $H_0$ and support the claim. At least 2 distributions are different.
ANOVA test and kruskal test have the same result, the main differences between them are:
