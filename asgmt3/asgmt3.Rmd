...
To avoid the influence of invalid data, we first remove outliers and NA data in column age.

```{r fig.height=3, fig.width=6, fig.align='center'}
life_data <- read.table("titanic.txt", header = T)
life_data <- subset(life_data, Age>=1)

par(mfrow=c(1, 2))
toClass <- xtabs(~PClass, data=life_data)
toSex <- xtabs(~Sex, data=life_data)
barplot(xtabs(Survived~PClass, data=life_data)/toClass)
barplot(xtabs(Survived~Sex, data=life_data)/toSex)


sex_class <- xtabs(~Sex+PClass, data=life_data); sex_class
sex_class.c <- xtabs(Survived~Sex+PClass, data=life_data)
round(sex_class.c/sex_class,2)
```

We find:
1) The barplot shows the survival rate percentage per Sex level and per PClass level, we can conclude that higher passenger class has greater survival rate, and female passengers have higher survival rate than male passengers.
2) The first table shows the total numbers of individuals for each combination of levels of Sex and PClass, male and 3rd class passengers makes up the majority.
3) The second table shows the percentage of individuals with survived for every combination of levels of Sex and PClass, we can see that the higher class, the greater the survival rate, and female passengers get higher survival rate. 
