...
```{r}
fly_data <- read.table("fruitflies.txt", header = T)
fly_data$loglongevity <- log(fly_data$longevity)
cols <- c("loglongevity", "activity", "thorax")
with(fly_data, {
...
```{r}
fly_lm <- lm(loglongevity~factor(activity), data = fly_data)
res <- anova(fly_lm)
coef <- summary(fly_lm)$coefficients[, "Estimate"]; coef
```
According to the test result, since we can notice that the $p < 0.05$, which rejects the null hypothesis, the sexual activity has impact on fly's longevity. The estimated longevities of high is `r coef["(Intercept)"]`, low is `r coef["(Intercept)"] + coef["factor(activity)low"]`, isolated is `r coef["(Intercept)"] + coef["factor(activity)isolated"]`.
So, the result indicates that less sexual activity could lead to higher longevity.
...
```{r}
fly_lm1 <- lm(loglongevity~factor(activity) + thorax, data = fly_data)
drop1(fly_lm1, test="F")
```
Accroding to the test, we can notice that 2 factors $p< 0.05$. Hence, $H_0$ is rejected. Both factors have great influence on longevity.

```{r}
coef <- summary(fly_lm1)$coefficients[, "Estimate"]; coef
```

As we can see, the estimated longevities decrease with sexual activity.
The estimated longevities for group 'low' is `r coef["(Intercept)"] + coef["factor(activity)low"] +  coef["thorax"]*mean(fly_data[,"thorax"]) `, isolated is `r coef["(Intercept)"] + coef["factor(activity)isolated"] +  coef["thorax"]*mean(fly_data[,"thorax"]) `, and high is `r coef["(Intercept)"] + coef["thorax"]*mean(fly_data[,"thorax"]) `, for average thorax length.
...
Accroding to the result from b) and the plot from a), we can conclude that longer the thorax, longer the longevity.

To test whether this dependence is similar under all three conditions of sexual activity. We make an  interaction plot.

```{r}
with(fly_data, {
  interaction.plot(thorax, factor(activity), loglongevity)
})
```
...
We would like to choose the analyses with thorax length, since the p-value of thorax is less than 0.05, indicating it has effect on longevity. Hence, we believe that the analyses without considering thorax should be wrong.
...
Left plot: the line is straight. Right plot: OK. Therefore, we concludes that the normality and heteroscedasticity is normal.
